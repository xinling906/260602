...
private:
    ...
    // 洗牌後的 52 張順序，每個元素 0~51
    std::vector<int> deckOrder_;
    size_t deckPos_ = 0;

    // 是否在這 52 張過程中曾出現過任何一次「牌面 == 右側文字」
...
private:
    ...
    int RandomIntervalMs() {
        std::uniform_int_distribution<int> dist(1000, 1000); // 截圖中顯示 dist(100, 1000) 亦可依需求調整
        return dist(rng_);
    }
    ...
    bool ShowNextItem() {
        ...
        currentSuitIndex_ = cardId / 13; // 0~3
        currentValueIndex_ = cardId % 13; // 0~12
    ...
    LRESULT HandleMessage(UINT msg, WPARAM wParam, LPARAM lParam) {
        ...
        case WM_COMMAND:
            ...
            case ID_HELP_ABOUT:
                MessageBoxW(
                    hwnd_,
                    L"玩法：1. 按【開始】 2. 52 張牌會先洗牌再依序顯示 3. 若牌面與右側數字 / JQK 一致，按空白鍵 4. 若 52 張全部都沒有符合則顯示失敗",
                    L"說明",
        ...
        case WM_TIMER:
            if (wParam == TIMER_ID && running_) {
                ...
                if (deckPos_ < deckOrder_.size()) {
                    ...
                    // 最後一張已經顯示過且時間到，檢查是否 52 張都沒有任何符合
